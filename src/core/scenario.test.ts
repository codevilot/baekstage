...
const graph = defineScenario({
  ...
  nodes: [
    ...
    { id: "collector", title: "Collector screen", kind: "screen", facets: { role: ["Collector"] } },
  ],
  edges: [
    { id: "a", source: "fixture", target: "admin" },
    { id: "b", source: "fixture", target: "collector" },
  ],
});
...
describe("scenario graph", () => {
  ...
  it("captures and marks a locator without requiring Playwright at runtime", async () => {
    ...
    await markElementScreenshot(
      ...
      { label: "KPI card", nodeId: "kpi-after", target: "[data-testid=kpi]" },
    );
    expect(readScreenshotMark(calls[0])).toMatchObject({ nodeId: "kpi-after", target: "[data-testid=kpi]" });
  });
  ...
  it("keeps same-named nodes separated by scenario id", () => {
    const shot = { label: "collector", url: "/collector.png", type: "screenshot" as const, scenarioId: "lifecycle-2", nodeId: "collector" };
    const lifecycle2 = { id: "lifecycle-2:projection-collector", source: "lifecycle-2:projection", target: "lifecycle-2:collector" };
    const lifecycle3 = { id: "lifecycle-3:projection-collector", source: "lifecycle-3:projection", target: "lifecycle-3:collector" };
  ...
  it("opens only screenshots directly marked for the selected node", () => {
    const shots = [
      { label: "before", url: "/before.png", type: "screenshot" as const, nodeId: "kpi-before" },
      { label: "after", url: "/after.png", type: "screenshot" as const, nodeId: "kpi-after" },
    ];
    expect(screenshotsForNode("kpi-before", shots).map((shot) => shot.label)).toEqual(["before"]);
  });
