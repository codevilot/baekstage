...
Use `markElementScreenshot` for a KPI card, table, dialog, or other focused region.
...
await test.step("Scenario node: KPI after", async () => {
  await markElementScreenshot(page.getByTestId("collector-kpi"), testInfo, {
    scenarioId: "sampling-collector",
    nodeId: "collector-after",
    label: "Collector KPI after sampling",
    category: "After result",
    target: "[data-testid=collector-kpi]",
    checkpoint: true,
  });
});
```
