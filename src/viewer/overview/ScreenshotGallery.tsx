...
export function ScreenshotGallery({ screenshots, focusUrls, traceViewerEndpoint }: { screenshots: ScenarioArtifact[]; focusUrls?: Set<string>; traceViewerEndpoint?: string }) {
  ...
  return <section className="screenshot-gallery" aria-label="Playwright screenshots">
    ...
    {hovered && !selected && createPortal(<div className="baekstage-portal"><div className="screenshot-hover-preview"><img src={hovered.url} alt=""/><span>{hovered.label}</span></div></div>, document.body)}
    {(selected?.domSnapshotUrl || selected?.traceUrl) && traceOpen && createPortal(<div className="baekstage-portal"><TraceSnapshotViewer artifact={selected} endpoint={traceViewerEndpoint} onClose={() => setTraceOpen(false)}/></div>, document.body)}
    {selected && !traceOpen && createPortal(<div className="baekstage-portal"><div className="screenshot-lightbox" role="dialog" aria-modal="true" aria-label={selected.label} onClick={() => setSelectedIndex(null)}><button className="lightbox-close" aria-label="Close screenshot">×</button><button className="lightbox-arrow previous" onClick={(event) => { event.stopPropagation(); move(-1); }} aria-label="Previous screenshot">‹</button><figure onClick={(event) => event.stopPropagation()}><img src={selected.url} alt={selected.label}/><figcaption><b>{selected.nodeNumber ? `#${selected.nodeNumber} · ${selected.nodeTitle} · ` : ""}{selected.category ?? "Screenshot"}{selected.branch ? ` · ${selected.branch}` : ""}</b><span>{selected.label}</span><div><button onClick={() => move(-1)}>← 이전</button><em>{(selectedIndex ?? 0) + 1} / {ordered.length}</em><button onClick={() => move(1)}>다음 →</button></div>{(selected.domSnapshotUrl || selected.traceUrl) && <button className="open-trace" onClick={() => setTraceOpen(true)}>Open interactive trace</button>}<button className="confirm-next" onClick={nextUnreviewed}>확인하고 다음 미확인 →</button></figcaption></figure><button className="lightbox-arrow next" onClick={(event) => { event.stopPropagation(); move(1); }} aria-label="Next screenshot">›</button></div></div>, document.body)}
  </section>;
}
