import type { NetworkStepMarker, ObservedNetworkRecord } from "../core/types";
...
export type BaekstageNetworkFixture = { records: ObservedNetworkRecord[]; step<T>(marker: string | NetworkStepOptions, action: () => Promise<T>): Promise<T>; markNode(nodeId: string): Promise<void>; flush(): Promise<void> };
...
export function observeApiScenario(page: PageLike, testInfo: TestInfoLike, options: ObserveApiScenarioOptions): BaekstageNetworkFixture {
  ...
  const records: ObservedNetworkRecord[] = []; const pending = new Set<Promise<void>>(); const inFlight = new Set<RequestLike>(); const started = new WeakMap<RequestLike, number>(); const byRequest = new WeakMap<RequestLike, ObservedNetworkRecord>(); const activeSteps: NetworkStepMarker[] = [];
  ...
  const flush = () => flushed ??= (async () => { page.off?.("request", onRequest); const deadline = Date.now() + (options.flushTimeoutMs ?? 1_000); while ((inFlight.size || pending.size) && Date.now() < deadline) await new Promise((resolve) => setTimeout(resolve, 10)); page.off?.("response", onResponse); page.off?.("requestfailed", onFailed); activePages.delete(page as object); await Promise.allSettled([...pending]); const safe = redactValue(records, options.redactKeys); await testInfo.attach(`${NETWORK_ATTACHMENT_PREFIX}${encodeURIComponent(JSON.stringify({ scenarioId: options.scenarioId }))}`, { body: Buffer.from(JSON.stringify(safe)), contentType: "application/json" }); })();
  return { records, async step<T>(marker: string | NetworkStepOptions, action: () => Promise<T>) { const value: NetworkStepMarker = typeof marker === "string" ? { id: marker } : marker; activeSteps.push(value); try { return await action(); } finally { activeSteps.splice(activeSteps.lastIndexOf(value), 1); } }, async markNode(nodeId: string) { for (const record of records.slice(markCursor)) if (!record.step) record.step = { id: `mark:${nodeId}`, toNodeId: nodeId }; markCursor = records.length; }, flush };
}
