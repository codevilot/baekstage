...
export function ScenarioRunPanel({ result, running, error, disabled, hideAction, onRun }: { result: ScenarioRun | null; running: boolean; error: string | null; disabled?: boolean; hideAction?: boolean; onRun: () => void }) {
  return <section className="scenario-run-panel" aria-live="polite">
    ...
    {error && <p>{error}</p>}
    {!hideAction && <button onClick={onRun} disabled={disabled || running}>{running ? <><i/> Playwright 전체 실행 중…</> : result ? "전체 테스트 다시 실행" : "전체 Playwright 테스트 실행"}</button>}
