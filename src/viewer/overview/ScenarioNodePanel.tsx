...
export function ScenarioNodePanel({ scenario, node, screenshots, traceViewerEndpoint, onBack, onSelect, onRunApi }: { scenario: ScenarioGraph; node: ScenarioNode; screenshots: ScenarioArtifact[]; traceViewerEndpoint?: string; onBack: () => void; onSelect: (nodeId: string) => void; onRunApi?: () => void }) {
  ...
  useEffect(() => { panelRef.current?.scrollTo({ top: 0, behavior: "auto" }); }, [node.id]);
  useEffect(() => {
    const navigate = (event: KeyboardEvent) => {
      const target = event.target as HTMLElement | null;
      if (target?.closest("input,textarea,select,[contenteditable=true]") || document.querySelector(".screenshot-lightbox")) return;
